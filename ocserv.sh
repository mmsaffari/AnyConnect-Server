...
# Latest ocserv version can be found here: https://www.infradead.org/ocserv/download/
ocserv_ver="1.1.6"
...
check_root(){
    [[ $EUID != 0 ]] && echo -e "${Error} Current user is not root or doesn't have root acces, can't continue, please switch to root or use command: ${Green_background_prefix}sudo su${Font_color_suffix} to get a temp root privilege(may request user password)." && exit 1
}
...
Download_ocserv(){
    ...
    if [[ -e ${file} ]]; then
        mkdir "${conf_file}"
        wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/mmsaffari/AnyConnect-Server/main/ocserv.conf"
...
Service_ocserv(){
    if ! wget --no-check-certificate https://raw.githubusercontent.com/mmsaffari/AnyConnect-Server/main/ocserv_debian_service -O /etc/init.d/ocserv; then
...
Generate_SSL(){
    echo "Please input the DNS name of the VPN server"
    read -e -p "(Default: example.com):" DOMAIN
    [[ -z "${DOMAIN}" ]] && echo -e "${Error} Enter your server's DNS name!" && exit 1
    echo && echo -e "   Domain : ${Red_font_prefix}${DOMAIN}${Font_color_suffix}" && echo
    certbot certonly --standalone --agree-tos --no-eff-email --staple-ocsp --register-unsafely-without-email -d $DOMAIN
}
Installation_dependency(){
    ...
    if [[ ${release} = "centos" ]]; then
        ...
        if [[ $? = 0 ]]; then
            apt-get update
            apt-get install snapd vim net-tools pkg-config build-essential libgnutls28-dev libwrap0-dev liblz4-dev libseccomp-dev libreadline-dev libnl-nf-3-dev libev-dev gnutls-bin -y
            snap install core
            snap refresh core
            ...
            wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/mmsaffari/AnyConnect-Server/main/us.debian.sources.list"
            apt-get update
            apt-get install snapd vim net-tools pkg-config build-essential libgnutls28-dev libwrap0-dev liblz4-dev libseccomp-dev libreadline-dev libnl-nf-3-dev libev-dev gnutls-bin -y
            snap install core
            snap refresh core
...
Install_ocserv(){
    ...
    Service_ocserv
    echo -e "${Info} Start to get an SSL cert from Let's Encrypt..."
    Generate_SSL
...
Set_passwd(){
    ...
    read -e -p "(Default: P@\$\$w0rd):" userpass
    [[ -z "${userpass}" ]] && userpass="P@\$\$w0rd"
...
Set_Config(){
    ...
    sed -i "s_server-cert = .*_server-cert = /etc/letsencrypt/live/${DOMAIN}/fullchain.pem_g" ${conf}
    sed -i "s_server-key = .*_server-key = /etc/letsencrypt/live/${DOMAIN}/privkey.pem_g" ${conf}
}
Read_config(){
    [[ ! -e ${conf} ]] && echo -e "${Error} ocserv config file doesn't exist !" && exit 1
...
Update_Shell(){
    sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/mmsaffari/AnyConnect-Server/main/ocserv.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
